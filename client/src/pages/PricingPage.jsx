  ...
  const plans = [
    {
      ...
      price: "₹0",
      period: "for 14 days",
      desc: "Perfect for exploring Amexora features.",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 flex-1">
        ...
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-4">
          ...
          <div className="pt-4 flex items-center justify-center gap-3">
            ...
            <span className={`text-sm font-medium ${annualBilling ? 'text-slate-900 dark:text-white' : 'text-slate-500'}`}>
              Annual <span className="text-xs font-bold text-emerald-500 bg-emerald-100 dark:bg-emerald-950 px-2 py-0.5 rounded-full">Save 20%</span>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {plans.map((p, i) => (
            <div
              key={i}
              className={`p-6 rounded-3xl bg-white dark:bg-slate-900 border flex flex-col justify-between relative shadow-card ${
                p.popular ? 'border-blue-600 ring-2 ring-blue-600/20' : 'border-slate-200 dark:border-slate-800'
              }`}
            >
              {p.popular && (
                <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-3 py-1 bg-blue-600 text-white text-[11px] font-bold rounded-full uppercase tracking-wider flex items-center gap-1 shadow-md">
